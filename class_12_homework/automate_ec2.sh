...
instacne_id=$(aws ec2 run-instances --launch-template '{"LaunchTemplateId":$instacne_id}' | grep -n "InstanceId" | awk '{print $3}' | grep -o "[a-z].*.[0-9]"))
...
public_ip=$(aws ec2 describe-instances --instance-ids $instacne_id | grep -n "PublicIp" | awk '{print $3}' | grep -0 "[0-9].*.[0-9]" )
