...
check_bucket= $(aws s3 ls | awk '/$bucket_dest/ {print $1 }')

# if not present creates the bucket
if [[ $check_bucket != $bucket_dest ]];
then
   aws s3api create-bucket --bucket $2 --region us-east-1
fi
...
if [[ $status_bucket = 'false' ]];
then
   aws s3api delete-public-access-block --bucket $check_bucket
fi

# attaches the policy to the bucket
aws s3api put-bucket-policy --bucket $check_bucket --policy ./acl_policy.json

# configures the bucket website
aws s3api put-bucket-website --bucket $check_bucket --website-configuration ./website_conf.json
